...
evaluate_fitness <- function(generation_t0, Y, X, family, objFun,
                             parallel, minimize, 
                             calc_objective_function, rank_objective_function) {
...
create_next_generation <- function(generation_t0, objFunOutput_t0,
    ...
    while(i <= dim(generation_t1)[1]) {
        ...
        # Mutation ----------------
        child1 <- mutate_child(mutation_rate, children[1, ])
        child2 <- mutate_child(mutation_rate, children[2, ])
...
output.meth1.all <- GAfun(y, x, iter = 100, objFun = "AIC", parallel = F,
                      crossMeth = "method1", converge = F, family = "gaussian",
                      pCrossover = 0.9)
output.meth2 <- GAfun(y, x, iter = 100, objFun = "AIC", parallel = F,
                          crossMeth = "method2", converge = T, family = "gaussian",
                          pCrossover = 0.9)
output.meth2.all <- GAfun(y, x, iter = 100, objFun = "AIC", parallel = F,
                      crossMeth = "method2", converge = F, family = "gaussian",
                      pCrossover = 0.9)
output.meth3 <- GAfun(y, x, iter = 100, objFun = "AIC", parallel = F,
                          crossMeth = "method3", converge = T, family = "gaussian",
                          pCrossover = 0.9)
output.meth3.all <- GAfun(y, x, iter = 100, objFun = "AIC", parallel = F,
                      crossMeth = "method3", converge = F, family = "gaussian",
                      pCrossover = 0.9)
